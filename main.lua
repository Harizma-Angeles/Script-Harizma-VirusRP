-- HARIZMA CYBER HUB [ESP & FOV FIXED VERSION + CHAT SPY]
...
Players.PlayerAdded:Connect(function(plr)
    plr.Chatted:Connect(function(msg)
        if not chatSpyEnabled or not plr.Character then return end
        ...
        if head then
            ...
            lbl.TextSize = 18
            lbl.Text = plr.Name .. ": " .. msg
            task.delay(4, function() bg:Destroy() end)
        end
    end)
end)

-- [Остальная часть твоего кода остается без изменений]
local fovCircle = nil
pcall(function()
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if fovCircle then
        fovCircle.Position = mouseLocation fovCircle.Radius = fovRadius fovCircle.Visible = aimbotEnabled fovCircle.Color = themeColor 
    end
    if aimbotEnabled and UserInputService:IsMouseButtonPressed(aimButton) then 
        ...
        if closestPlayer then 
            ...
            if head then camera.CFrame = camera.CFrame:Lerp(CFrame.new(camera.CFrame.Position, head.Position), aimSmoothness) end
        end 
    end 
...
-- ЧАТ
createToggle(frames["ЧАТ"], "Видеть сообщения", 10, nil, function(v) chatSpyEnabled = v end)
